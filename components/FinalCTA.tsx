...
function StoreComingSoon({ platform }: { platform: 'ios' | 'android' }) {
  const isIos = platform === 'ios'
  return (
    <div className="flex items-center gap-3 px-6 py-4 bg-white/10 border border-white/20 text-white/50 rounded-2xl cursor-default">
      {isIos ? (
      ...
      <div className="text-left leading-tight">
        <div className="text-[10px] font-body">{isIos ? 'App Store' : 'Google Play'}</div>
        <div className="text-base font-heading font-bold">Coming soon</div>
      </div>
    </div>
  )
}
  ...
  return (
    <section id="download" className="relative py-28 md:py-40 overflow-hidden">
      ...
      <div className="relative max-w-4xl mx-auto px-5 sm:px-8 text-center">
        ...
        >
          <StoreComingSoon platform="ios" />
          <StoreComingSoon platform="android" />
        </motion.div>
        ...
        >
          Launching first in Nairobi — <a href="mailto:support@fastaapp.co?subject=Waitlist" className="underline text-white/70 hover:text-white transition-colors">join the waitlist</a>.
        </motion.p>
      </div>
    </section>
  )
